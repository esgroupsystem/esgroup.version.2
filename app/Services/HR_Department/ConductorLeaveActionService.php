...
class ConductorLeaveActionService
{
    ...
    private function markSecondNotice(
        ...
        return '2nd Notice marked as sent. The conductor is now Inactive.';
    }
    ...
    private function markFinalNotice(
        ...
        return 'Final Notice marked as sent. The conductor is now Terminated.';
    }

    private function cancelLeave(ConductorLeave $leave, ?string $note): string
    {
        ...
        return 'Leave cancelled. The conductor returned to Active.';
    }

    private function markReadyForDuty(ConductorLeave $leave, ?string $note): string
    {
        DB::transaction(function () use ($leave, $note): void {
            $record = $this->lockLeave($leave->id);
        ...
        return 'Conductor marked as Ready for Duty and returned to Active.';
    }

    private function lockLeave(int $leaveId): ConductorLeave
    {
        return ConductorLeave::query()
            ->with('employee')
            ->lockForUpdate()
            ->findOrFail($leaveId);
    }

    private function assertOpen(ConductorLeave $leave): void
    {
        if ($leave->isClosed()) {
