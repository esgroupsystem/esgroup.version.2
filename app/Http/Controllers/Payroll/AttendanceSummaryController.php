    ...
    protected function storeSummary(
        ...
        if ($isHoliday) {
            ...
            if ($workedMinutes >= self::FULL_DAY_MINUTES) {
                ...
                $rawUndertimeMinutes = max(0, self::FULL_DAY_MINUTES - $workedMinutes);
                $undertimeMinutes = $this->roundedUndertimeDeductionMinutes($rawUndertimeMinutes);
                $payableMinutes = max(0, self::FULL_DAY_MINUTES - $undertimeMinutes);

                [$payableDays, $payableHours] = $this->payUnitsFromMinutes($payableMinutes);

                $remarks[] = 'Flexible shift below 9 clock hours. Undertime rounded using company block rule.';
            }
        } else {
            ...
            if ($attendanceStatus === 'present') {
                ...
                $remarks[] = 'Late/undertime deducted from 8 paid hours. Undertime uses 5-minute grace and 30-minute deduction blocks.';
            }
        }
