...
class DriverLeaveController extends Controller
{
    public function index(Request $request): View
    {
        ...
        $garageSummary = $allForCounts
            ->groupBy(fn (DriverLeave $leave) => $leave->employee?->garage ?: 'No Garage Assigned')
        ...
        return view('hr_department.leaves.driver.index', compact(
            'leaves',
            'counts',
            'today',
            'garageSummary'
    ...
    public function update(Request $request, DriverLeave $leave): RedirectResponse
    {
        $validated = $request->validate([
            'employee_id' => ['required', 'exists:employees,id'],
            'leave_type' => ['required', 'string', 'max:100'],
    ...
    public function action(Request $request, DriverLeave $leave): RedirectResponse
    {
    ...
    private function markFirstNotice(DriverLeave $leave, ?string $note): RedirectResponse
    {
    ...
    private function markSecondNoticeAndDeactivate(DriverLeave $leave, ?string $note): RedirectResponse
    {
        if (! $leave->first_notice_sent_at) {
        ...
        flash('2nd Notice marked as sent. Driver record is now automatically Inactive.')->success();

        return redirect()->route('driver-leave.driver.index');
    }

    private function markFinalNoticeAndTerminate(DriverLeave $leave, ?string $note): RedirectResponse
    {
        if (! $leave->second_notice_sent_at) {
        ...
        flash('Final Notice marked as sent. Driver record is now Terminated.')->success();

        return redirect()->route('driver-leave.driver.index');
    }

    private function cancelLeave(DriverLeave $leave, ?string $note): RedirectResponse
    {
        DB::transaction(function () use ($leave, $note) {
            $leave->update([
                'status' => 'Cancelled',
    ...
    private function markReadyForDuty(DriverLeave $leave, ?string $note): RedirectResponse
    {
        DB::transaction(function () use ($leave, $note) {
            $leave->update([
                'status' => 'Completed',
                'offense_level' => 0,
                'ready_for_duty_notified_at' => now(),
                'last_action_note' => $note,
            ]);
