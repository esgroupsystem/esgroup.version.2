...
class ProcessEmployeeLeaveReadyForDuty extends Command
{
    ...
    public function handle(LeaveNotificationService $notificationService)
    {
        ...
        foreach ($leaves as $leave) {
            if (empty($leave->end_date)) {
                $this->line("Leave ID {$leave->id}: skipped, no end_date");
            ...
            if ($today->lte($end)) {
                $this->line("Leave ID {$leave->id}: skipped, today <= end_date");
            ...
            if ($daysAfterEnd == 1 && is_null($leave->ready_for_duty_notified_at)) {
                $this->info("Sending Ready for Duty for leave ID {$leave->id}");
            ...
            if ($daysAfterEnd >= 2 && $daysAfterEnd <= 9 && is_null($leave->first_notice_sent_at)) {
                $this->info("Sending 1st Notice for leave ID {$leave->id}");
            ...
            if ($daysAfterEnd >= 10 && $daysAfterEnd <= 22 && is_null($leave->second_notice_sent_at)) {
                $this->info("Sending 2nd Notice for leave ID {$leave->id}");
            ...
            if ($daysAfterEnd >= 23 && is_null($leave->final_notice_sent_at)) {
                $this->info("Sending 3rd Notice / Subject for Termination for leave ID {$leave->id}");
            ...
            $this->line("Leave ID {$leave->id}: no notice triggered");
            $this->line(str_repeat('-', 50));
        }

        $this->info('Employee leave notices processed.');
