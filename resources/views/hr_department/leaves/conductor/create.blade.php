...
@section('content')
    <div class="container" data-layout="container">
        <div class="content conductor-leave-form-page">
            ...
            <div class="card mb-4 overflow-hidden">
                ...
                <div class="card-body position-relative">
                    <div class="row align-items-center g-3">
                        <div class="col-lg-8">
                            <h3 class="mb-1">Create Conductor Leave</h3>
                            <p class="text-700 mb-0">
                                Record conductor leave details with garage visibility and notice workflow tracking.
                            </p>
                        </div>

                        <div class="col-lg-4 text-lg-end">
                            <a href="{{ route('conductor-leave.conductor.index') }}" class="btn btn-falcon-default">
            ...
            <form action="{{ route('conductor-leave.conductor.store') }}" method="POST">
                ...
                <div class="row g-4">
                    <div class="col-xl-8">
                        <div class="card">
                                ...
                                <div class="d-flex justify-content-between align-items-start gap-3">
                                    <div>
                                        ...
                                        <p class="mb-0 text-600 small">
                                            Select the conductor and encode leave period details.
                                        </p>
                                    </div>
                                    ...
                                    <div class="col-md-12">
                                        ...
                                        <select name="employee_id" id="employee_id"
                                            ...
                                            @foreach ($conductors as $conductor)
                                                <option value="{{ $conductor->id }}" data-name="{{ $conductor->full_name }}"
                                                    data-position="{{ $conductor->position?->title ?? 'No position' }}"
                                                    data-garage="{{ $conductor->garage ?? 'No Garage Assigned' }}"
                                                    data-company="{{ $conductor->company ?? 'No Company' }}"
                                                    data-status="{{ $conductor->status ?? 'No Status' }}"
                                                    data-employee-no="{{ $conductor->employee_id_permanent ?? $conductor->employee_id }}"
                                                    {{ old('employee_id') == $conductor->id ? 'selected' : '' }}>
                                                    {{ $conductor->full_name }}
                                                    | {{ $conductor->garage ?? 'No Garage' }}
                                                    | {{ $conductor->position?->title ?? 'Conductor' }}
                                                </option>
                                            @endforeach
                                        </select>
                                        ...
                                        <div class="form-text">
                                            Garage is displayed beside each conductor for easier assignment checking.
                                        </div>
                                    </div>
                            ...
                            <div class="card-footer bg-body-tertiary text-end">
                                <a href="{{ route('conductor-leave.conductor.index') }}"
                                    class="btn btn-falcon-default me-2">
                                    Cancel
                                </a>
                    ...
                    <div class="col-xl-4">
                        <div class="card mb-3">
                            <div class="card-header bg-body-tertiary">
                                <h5 class="mb-0">Selected Conductor Details</h5>
                            </div>

                            <div class="card-body">
                                <div class="conductor-preview-empty text-center py-4" id="conductorPreviewEmpty">
                                    <div class="preview-icon mb-2">
                                        <span class="fas fa-user"></span>
                                    </div>
                                    <p class="text-600 mb-0">
                                        Select a conductor to view garage and employee details.
                                    </p>
                                </div>

                                <div id="conductorPreview" class="d-none">
...
@push('scripts')
    <script>
        window.addEventListener('load', function() {
            ...
            function updateConductorPreview() {
                const selected = employeeSelect.options[employeeSelect.selectedIndex];

                const preview = document.getElementById('conductorPreview');
                const empty = document.getElementById('conductorPreviewEmpty');
            ...
            if (typeof jQuery !== 'undefined' && typeof jQuery.fn.select2 !== 'undefined') {
                ...
                select.select2({
                    width: '100%',
                    placeholder: 'Search conductor by name, garage, or position...',
                    allowClear: true,
                ...
                select.on('change', updateConductorPreview);
            } else {
                employeeSelect.addEventListener('change', updateConductorPreview);
            }
...
@push('styles')
    <style>
        .conductor-leave-form-page .preview-icon {
            width: 3rem;
            height: 3rem;
            border-radius: 50%;
            display: inline-grid;
        ...
        .conductor-leave-form-page .detail-row {
            display: flex;
        ...
        .conductor-leave-form-page .detail-row:last-child {
            border-bottom: 0;
        }

        .conductor-leave-form-page .detail-row span {
