...
@section('content')
    <div class="container" data-layout="container">
        ...
        <div class="content driver-leave-page">
            @if ($errors->any())
                    ...
                    <div class="row align-items-center g-3">
                        <div class="col-lg-8">
                            <div class="d-flex align-items-center gap-3">
                                <div class="driver-page-icon">
                                    <span class="fas fa-user-clock"></span>
                                </div>

                                <div>
                                    <h3 class="mb-1">Driver Leave Monitoring</h3>
                        ...
                        <div class="col-lg-4 text-lg-end">
                            <a href="{{ route('driver-leave.driver.create') }}" class="btn btn-primary">
                ...
                <div class="col-sm-6 col-xl-3">
                    <div class="card metric-card h-100">
                        <div class="card-body">
                            ...
                            <div class="small text-600 mt-2">Drivers with first notice</div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card h-100">
                        <div class="card-header bg-body-tertiary">
                            <div class="d-flex justify-content-between align-items-center">
                                ...
                                <span class="badge badge-subtle-info text-info">Per Driver Garage</span>
                            </div>
                        </div>
                    ...
                    <div class="card h-100">
                        ...
                        <div class="card-body">
                            <div class="row g-3">
                                ...
                                <div class="col-md-4">
                                    <div class="workflow-step border rounded-3 p-3 h-100">
                                        ...
                                        <p class="text-700 small mb-0">
                                            Upload proof and automatically set the driver to Inactive.
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-4">
                                    <div class="workflow-step border rounded-3 p-3 h-100">
                                        ...
                                        <p class="text-700 small mb-0">
                                            Upload final proof and set the driver to Terminated.
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="small text-700">
                                <strong>Note:</strong> Use Ready for Duty only after the driver has returned and is cleared
                                to work.
                            </div>
                        </div>
                    </div>
            ...
            <div class="card mb-4">
                <div class="card-header bg-body-tertiary">
                    <div class="row g-3 align-items-center">
                        <div class="col-md-7">
                            <h5 class="mb-0">Driver Leave Records</h5>
                        ...
                        <div class="col-md-5">
                            <div class="input-group">
                                ...
                                <input id="liveSearch" class="form-control"
                                    placeholder="Search driver, garage, company, status, leave type..."
                ...
                <div class="card-body p-0">
                    <div id="driverLeaveTable">
                        @include('hr_department.leaves.driver.table')
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="modal fade" id="driverLeaveActionModal" tabindex="-1" aria-labelledby="driverLeaveActionModalLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form id="driverLeaveActionForm" method="POST" action="" enctype="multipart/form-data">
                @csrf

                <div class="modal-content border-0 shadow">
                    <div class="modal-header bg-body-tertiary">
                        <div>
                            <h5 class="modal-title" id="driverLeaveActionModalLabel">Confirm Action</h5>
                            <p class="mb-0 small text-600" id="driverActionSubtitle">
                    ...
                    <div class="modal-body">
                        <input type="hidden" name="action_type" id="driverActionType" value="">

                        <div class="driver-modal-box mb-3">
                            <div class="fw-bold text-900" id="driverActionEmployee"></div>
                            <div class="small text-600" id="driverActionLeaveType"></div>
                            <div class="small text-600" id="driverActionGarage"></div>
                        </div>

                        <div class="mb-3">
                            <label for="driverActionNote" class="form-label fw-semi-bold">
                                Action Note
                            </label>
                            <textarea name="note" id="driverActionNote" class="form-control" rows="4"
                                placeholder="Enter notice reference, HR note, or reason."></textarea>
                        </div>

                        <div class="mb-3 d-none" id="driverProofContainer">
                            <label for="driverProofImage" class="form-label fw-semi-bold">
                                Picture Proof <span class="text-danger">*</span>
                            </label>

                            <input type="file" name="proof_image" id="driverProofImage" class="form-control"
                            ...
                            <div class="proof-preview-wrapper d-none mt-3" id="driverProofPreviewWrapper">
                                <img src="" alt="Selected proof preview" id="driverProofPreview"
                                    class="proof-preview-image">
                            </div>
                        </div>

                        <div class="alert alert-warning small mb-0" id="driverActionWarning">
                    ...
                    <div class="modal-footer bg-body-tertiary">
                        ...
                        <button type="submit" class="btn btn-primary" id="driverActionSubmit">
                            Confirm
                        </button>
                    </div>
                </div>
...
@push('scripts')
    <script>
        (() => {
            const bootDriverLeavePage = () => {
                if (window.__driverLeavePageBooted) {
                    return;
                }

                const modalElement = document.getElementById('driverLeaveActionModal');
                const form = document.getElementById('driverLeaveActionForm');
                const tableContainer = document.getElementById('driverLeaveTable');

                if (!modalElement || !form || !tableContainer) {
                    console.error('Driver leave page is missing the modal, form, or table container.');
                    return;
                }

                if (typeof bootstrap === 'undefined' || !bootstrap.Modal) {
                    window.setTimeout(bootDriverLeavePage, 150);
                    return;
                }

                window.__driverLeavePageBooted = true;

                const modal = bootstrap.Modal.getOrCreateInstance(modalElement);
                const byId = (id) => document.getElementById(id);

                const fields = {
                    actionType: byId('driverActionType'),
                    title: byId('driverLeaveActionModalLabel'),
                    subtitle: byId('driverActionSubtitle'),
                    employee: byId('driverActionEmployee'),
                    leaveType: byId('driverActionLeaveType'),
                    garage: byId('driverActionGarage'),
                    note: byId('driverActionNote'),
                    proofContainer: byId('driverProofContainer'),
                    proofInput: byId('driverProofImage'),
                    previewWrapper: byId('driverProofPreviewWrapper'),
                    preview: byId('driverProofPreview'),
                    submit: byId('driverActionSubmit')
                };

                if (!fields.actionType || !fields.submit) {
                    console.error('Driver leave action fields are incomplete.');
                    return;
                }

                let warning = byId('driverActionWarning');

                if (!warning) {
                    warning = document.createElement('div');
                    warning.id = 'driverActionWarning';
                ...
                const actionConfig = {
                    first: {
                        ...
                        warning: 'The first warning will be recorded. The driver remains active or on leave.',
                        requiresProof: true
                    },
                    second: {
                        title: 'Mark 2nd Notice Sent',
                        subtitle: 'Record the second warning and set the driver to Inactive.',
                    ...
                    terminate: {
                        ...
                        subtitle: 'Record the final warning and terminate the driver record.',
                    ...
                    cancel: {
                        title: 'Cancel Leave',
                        subtitle: 'Cancel the leave and return the driver to Active.',
                        submit: 'Cancel Leave',
                        buttonClass: 'btn btn-secondary',
                        alertClass: 'alert alert-secondary small mb-0',
                        warning: 'No picture proof is required. The driver will return to Active.',
                        requiresProof: false
                    },
                    ready: {
                        title: 'Mark Ready for Duty',
                        subtitle: 'Complete the leave and return the driver to Active.',
                ...
                const openActionModal = (button) => {
                    ...
                    if (!config || !url) {
                        console.error('Invalid driver leave action button.', {
                            action,
                            url
                        });
                        return;
                    ...
                    if (fields.employee) {
                        fields.employee.textContent = button.dataset.employee || 'No driver name';
                    }
                ...
                const loadTable = async (url) => {
                    ...
                    if (!response.ok) {
                        throw new Error('Unable to refresh driver leave records.');
                    }
                ...
                document.addEventListener('click', (event) => {
                    const actionButton = event.target.closest('.driver-action-open-modal');
                    ...
                    const paginationLink = event.target.closest('#driverLeaveTable .pagination a');
                ...
                form.addEventListener('submit', (event) => {
                    ...
                    if (!actionUrl || !fields.actionType.value) {
                        event.preventDefault();
                        console.error('Driver leave form action is incomplete.');
                        return;
                    }
            ...
            if (document.readyState === 'loading') {
                document.addEventListener('DOMContentLoaded', bootDriverLeavePage, {
                    once: true
                });
            } else {
                bootDriverLeavePage();
...
@push('styles')
    <style>
        .driver-leave-page .driver-page-icon {
            width: 3.25rem;
            height: 3.25rem;
            border-radius: 1rem;
            display: grid;
        ...
        .driver-leave-page .metric-card {
            border: 0;
            box-shadow: 0 .125rem .375rem rgba(0, 0, 0, .05);
        }

        .driver-leave-page .metric-icon {
            width: 2.5rem;
            height: 2.5rem;
            border-radius: .75rem;
            display: grid;
        ...
        .driver-leave-page .workflow-step {
            background: var(--falcon-gray-100, #f9fafd);
        }
        ...
        .driver-leave-page .pagination {
            font-size: 14px !important;
        }

        .driver-leave-page .pagination .page-link {
        ...
        .driver-leave-page .pagination .page-item.active .page-link {
            color: #fff !important;
        ...
        .driver-leave-page .pagination .page-link:hover {
            background: #e2e6ea !important;
            border-color: #c4c9cf !important;
        }

        .driver-leave-page .pagination .page-item.disabled .page-link {
            opacity: .5 !important;
        }

        .driver-leave-page .pagination .page-item {
