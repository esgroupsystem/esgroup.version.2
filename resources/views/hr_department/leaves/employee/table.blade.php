<div class="table-responsive scrollbar">
    <table class="table table-sm table-hover align-middle fs-10 mb-0 w-100 employee-leave-table">
                        ...
                        <div class="d-flex align-items-center">
                            <div class="avatar avatar-xl me-2">
                                <div class="avatar-name rounded-circle bg-primary-subtle text-primary">
                                    <span>{{ strtoupper(substr($employee?->full_name ?? 'E', 0, 1)) }}</span>
                                </div>
                            </div>
                    ...
                    <td class="text-center">
                        <div class="dropdown font-sans-serif position-static">
                            ...
                            <ul class="dropdown-menu dropdown-menu-end">
                                <li>
                                    ...
                                    @else
                                        <a class="dropdown-item"
                                            href="{{ route('employee-leave.employee.edit', $leave) }}">
                                ...
                                @if ($canShowReady)
                                    <li>
                                        <a class="dropdown-item employee-action-open-modal" href="#"
                                            data-url="{{ route('employee-leave.employee.action', $leave) }}"
                                ...
                                @if ($canShowNotices)
                                    ...
                                    <li>
                                        ...
                                        @else
                                            <a class="dropdown-item employee-action-open-modal" href="#"
                                                data-url="{{ route('employee-leave.employee.action', $leave) }}"
                                                data-action="second" data-id="{{ $leave->id }}"
                                                data-employee="{{ $employee?->full_name ?? '' }}"
                                                data-type="{{ $leave->leave_type }}"
                                                data-garage="{{ $garage }}">
                                    ...
                                    <li>
                                        ...
                                        @else
                                            <a class="dropdown-item employee-action-open-modal" href="#"
                                                data-url="{{ route('employee-leave.employee.action', $leave) }}"
                                                data-action="terminate" data-id="{{ $leave->id }}"
                                                data-employee="{{ $employee?->full_name ?? '' }}"
                                                data-type="{{ $leave->leave_type }}"
                                                data-garage="{{ $garage }}">
                ...
                <tr>
                    <td colspan="8" class="text-center py-5">
                        <div class="empty-state">
                            ...
                            <h6 class="mb-1">No employee leave records found</h6>
                            <p class="text-600 mb-0">Create a new employee leave record or adjust your search.</p>
                        </div>
                    </td>
                </tr>
            @endforelse
...
<style>
    .employee-leave-table .notice-tracker {
        display: flex;
        flex-direction: column;
        gap: .75rem;
    }

    .employee-leave-table .notice-step {
        display: flex;
    ...
    .employee-leave-table .notice-dot {
        width: .65rem;
        height: .65rem;
        border-radius: 50%;
        margin-top: .25rem;
    ...
    .employee-leave-table .notice-step.done {
        color: var(--falcon-info, #27bcfd);
    }

    .employee-leave-table .notice-step.done .notice-dot {
        background: var(--falcon-info, #27bcfd);
    }

    .employee-leave-table .notice-step.inactive-step {
        color: var(--falcon-warning, #f5803e);
    }

    .employee-leave-table .notice-step.inactive-step .notice-dot {
        background: var(--falcon-warning, #f5803e);
    }

    .employee-leave-table .notice-step.final-step {
        color: var(--falcon-danger, #e63757);
    }

    .employee-leave-table .notice-step.final-step .notice-dot {
        background: var(--falcon-danger, #e63757);
    }

    .employee-leave-table .notice-proof-link {
        display: inline-flex;
        align-items: center;
        gap: .45rem;
    ...
    .employee-leave-table .notice-proof-thumb {
        width: 42px;
        height: 42px;
        object-fit: cover;
        border-radius: .5rem;
    ...
    .employee-leave-table .dropdown-item.disabled {
        pointer-events: none;
        opacity: .55;
    }
