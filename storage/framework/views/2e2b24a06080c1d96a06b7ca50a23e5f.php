<?php $__env->startSection('title', 'Mirasol Biometrics Logs - HR'); ?>
                ...
                <div class="card-body">
                    <form id="syncForm" class="row g-2 align-items-end">
                        ...
                        <div class="col-12 col-lg-6 d-flex gap-2">
                            ...
                            <a class="btn btn-outline-secondary btn-sm" href="<?php echo e(route('mirasol-logs.index')); ?>">Reset</a>
                        </div>
                    </form>
                </div>
            </div>
                    ...
                    <div class="d-flex flex-column flex-lg-row gap-2 align-items-lg-center justify-content-between">
                        <div>
                            ...
                            <small class="text-muted">
                                Cutoff Coverage: <strong><?php echo e($cutoffLabel); ?></strong>
                            </small>
                        </div>

                        <form method="GET" action="<?php echo e(route('mirasol-logs.index')); ?>" class="row g-2 align-items-center">
                            <div class="col-auto">
                                <input type="text" name="q" list="employeeSuggestions"
                                    class="form-control form-control-sm" style="width: 260px;"
                                    placeholder="Search employee name / employee no..." value="<?php echo e(request('q')); ?>"
                                    required>
                                ...
                                <datalist id="employeeSuggestions">
                                    <?php $__currentLoopData = $people; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $p): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <?php if(!empty($p['employee_name'])): ?>
                                            <option value="<?php echo e($p['employee_name']); ?>">
                                                <?php echo e($p['employee_name']); ?><?php echo e(!empty($p['employee_no']) ? ' - ' . $p['employee_no'] : ''); ?>

                                            </option>
                                        <?php endif; ?>

                                        <?php if(!empty($p['employee_no'])): ?>
                                            <option value="<?php echo e($p['employee_no']); ?>">
                                                <?php echo e($p['employee_name']); ?>
                            ...
                            <?php $__empty_1 = true; $__currentLoopData = $rows; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $r): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                <tr>
                                    <td class="ps-3 text-muted">
                                        <?php echo e(($rows->firstItem() ?? 0) + $i); ?>

                                    </td>

                                    <td class="fw-semi-bold">
                                        <?php echo e($r['employee_name'] ?? '—'); ?>

                                        <?php if(!empty($r['shift_name'])): ?>
                                            <div class="text-muted fs-11"><?php echo e($r['shift_name']); ?></div>
                                        <?php endif; ?>
                                    </td>

                                    <td class="text-muted">
                                        <?php echo e($r['employee_no'] ?: '—'); ?>

                                    </td>

                                    <td class="text-muted">
                                        <?php echo e($r['log_date'] ? \Carbon\Carbon::parse($r['log_date'])->format('F d, Y (l)') : '—'); ?>

                                    </td>

                                    <td>
                                        <?php if(($r['schedule_status'] ?? null) === 'scheduled'): ?>
                                            <div class="fw-semibold text-success">
                                                <?php echo e(!empty($r['scheduled_time_in']) ? \Carbon\Carbon::parse($r['scheduled_time_in'])->format('h:i A') : '—'); ?>

                                                -
                                                <?php echo e(!empty($r['scheduled_time_out']) ? \Carbon\Carbon::parse($r['scheduled_time_out'])->format('h:i A') : '—'); ?>

                                            </div>
                                            <div class="fs-11 text-muted">
                                                Grace: <?php echo e($r['grace_minutes'] ?? 15); ?> min
                                            </div>
                                        <?php elseif(($r['schedule_status'] ?? null) === 'rest_day'): ?>
                                            <span class="badge bg-warning-subtle text-warning border">Rest Day</span>
                                        <?php elseif(($r['schedule_status'] ?? null) === 'leave'): ?>
                                            <span class="badge bg-info-subtle text-info border">Leave</span>
                                        <?php elseif(($r['schedule_status'] ?? null) === 'holiday'): ?>
                                    ...
                                    <td>
                                        <?php echo e(!empty($r['actual_time_in']) ? \Carbon\Carbon::parse($r['actual_time_in'])->format('h:i A') : '—'); ?>

                                    </td>

                                    <td>
                                        <?php echo e(!empty($r['actual_time_out']) ? \Carbon\Carbon::parse($r['actual_time_out'])->format('h:i A') : '—'); ?>

                                    </td>

                                    <td class="fw-semibold">
                                        <?php echo e($r['worked_hours_label'] ?? '—'); ?>
                                    ...
                                    <td>
                                        <?php if(($r['undertime_minutes'] ?? 0) > 0): ?>
                                            <span class="badge bg-danger-subtle text-danger border">
                                                <?php echo e($r['undertime_label']); ?>
                                    ...
                                    <td>
                                        <div
                                            class="small border rounded px-2 py-1 bg-<?php echo e($r['attendance_class'] ?? 'secondary'); ?>-subtle text-<?php echo e($r['attendance_class'] ?? 'secondary'); ?>">
                                            <?php echo e($r['attendance_note'] ?? '—'); ?>
                ...
                <?php if($isSearch ?? false): ?>
                    <div class="card-footer bg-body-tertiary border-top border-200">
                        <div class="d-flex flex-column flex-md-row gap-2 justify-content-between align-items-md-center">
                            <small class="text-muted">
                                Showing <?php echo e($rows->firstItem() ?? 0); ?> to <?php echo e($rows->lastItem() ?? 0); ?> of
                                <?php echo e($rows->total()); ?>

                            </small>
                            <div class="ms-md-auto"><?php echo e($rows->links('pagination.custom')); ?></div>
                        </div>
                    </div>
                <?php endif; ?>
            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const poll = async (jobId) => {
                try {
                    const res = await fetch(
                        `<?php echo e(route('mirasol-logs.sync-status')); ?>?job=${encodeURIComponent(jobId)}`, {
                            headers: {
            ...
            form.addEventListener('submit', async (e) => {
                ...
                try {
                    const res = await fetch(`<?php echo e(route('mirasol-logs.sync-start')); ?>`, {
                        method: 'POST',
                        headers: {
                            'X-Requested-With': 'XMLHttpRequest'
                        },
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\esgroup.version.2\resources\views/hr_department/mirasol_logs/index.blade.php ENDPATH**/ ?>