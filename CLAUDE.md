...
- Employee ↔ biometrics link: `employees.employee_biometric_id` (unique, nullable). `App\Support\HR\BiometricLink` builds the option lists (name matches first, marked "Name match"), the profile summary and `assignEmployee()`. HR profile: "Biometrics" card + `components/hr/biometric-link-dialog.tsx` (PUT `employees.biometric-link.update`, `employees.update`). Biometric Employees: "HR: name" under the name in the list, "Linked HR employee" select (`hr_employee_id`) in the edit form. Relations: `Employee::biometric()`, `EmployeeBiometric::hrEmployee()`.
